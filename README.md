...
- 🧠 **文本向量化提取**：内嵌本地轻量级离线 BERT 模型引擎 (`all-MiniLM-L6-v2`)，在映射步骤中对标题、摘要及关键词等字段生成高维特征向量 (384维度)，并可按需支持 GPU 显存自动调度。
...
```bash
git clone https://github.com/thriaaaa/openalex-update-tackle.git
cd openalex-update-tackle
...
- `merge_settings`: 定义了生成的增量文件以及进行除重的 `full_data_dir`。
- `schema_mapping_settings`: 所有文件格式映射的步骤开关以及源数据和映射路径等。

### 3. 一键运行
**方式一：Windows 用户（推荐）**
直接双击项目的 `run_auto_workflow.bat`，即可按照交互层指引操作：
```text
...
# 仅指定其中的某几项映射逻辑重新执行（比如我想单独刷新一遍作者合作的图谱结构边表和所有的向量数据）
python openalex_auto_workflow.py --steps schema_mapping --mapping-steps 08 10 11
```

## 📝 常见问题 (FAQ)
- **Q: 提示显存 OOM 或 GPU 不可用怎么办？**
  A: 系统会在执行 `schema_mapping` 里的 `works_vector` 时对本地硬件自审。无需人工干预或配置，若无 GPU 或显存吃紧，系统会自动把推断切回纯 CPU 计算以兜底。
