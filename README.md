...
- 🕸️ **多数据库协同映射**：基于一套数据源，能同时洗出适用不同检索和图算法场景的定制结构表：
  - **Graph Vertex (图顶点)**: \uthors_v.csv\, \works_v.csv\, \	opics_v.csv  - **Graph Edges (图边)**: \work_topic_e.csv\, \work_author_e.csv\, \uthor_author_e.csv\ 等
  - **Document (文档模型)**: \uthor_doc.csv\, \work_doc.csv- 🧠 **文本向量化提取**：内嵌本地轻量级离线 BERT 模型引擎 (\ll-MiniLM-L6-v2\)，在映射步骤中对标题、摘要及关键词等字段生成高维特征向量 (384维度)，并可按需支持 GPU 显存自动调度。
...
### 3. 一键运行 
**方式一：Windows 用户（推荐）**
直接双击项目的 un_auto_workflow.bat\，即可按照交互层指引操作：
\\	ext
...
- **Q: 提示显存 OOM 或 GPU 不可用怎么办？** 
  A: 系统会在执行 \schema_mapping\ 里的 _works_vector\ 时对本地硬件自审。无需人工干预或配置，若无 GPU 或显存吃紧，系统会自动把推断切回纯 CPU 计算以兜底。
