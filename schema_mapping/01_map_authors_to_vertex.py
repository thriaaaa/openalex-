...
"""
Authors 模式映射脚本
将 authors.csv 映射为图数据库顶点格式 authors_v.csv
"""
...
class AuthorsVertexMapper:
    ...
    def __init__(self, config_file="mapping_config.json"):
        ...
        self.output_base_dir = Path(self.config.get('output_dir', '../vertex_data'))
        self.output_dir = self.output_base_dir 
        
        # 创建输出目录
        self.output_dir.mkdir(parents=True, exist_ok=True)
        
        logger.info(f"Authors顶点映射器初始化完成")
    ...
    def map_author_to_vertex(self, author_row):
        """将author行映射为顶点格式
        ...
        properties = {
            ...
            'works_count': author_row.get('works_count', '0'),
            'cited_by_count': author_row.get('cited_by_count', '0')
        }
    ...
    def process_authors(self, batch_size=100000):
        """处理authors文件并生成顶点文件"""
        logger.info(f"\n{'='*60}")
        logger.info(f"开始映射 authors.csv -> authors_v.csv")
        logger.info(f"{'='*60}")
        
        input_file = self.input_dir / 'authors.csv'
        output_file = self.output_dir / 'authors_v.csv'
            ...
            with open(input_file, 'r', encoding='utf-8') as inf:
                ...
                with open(output_file, 'w', encoding='utf-8', newline='') as outf:
                    ...
                    for i, row in enumerate(reader, 1):
                        try:
                            # 映射为顶点格式
                            vertex = self.map_author_to_vertex(row)
                            batch.append(vertex)
