...
"""
Topics 模式映射脚本
将 topics.csv 映射为图数据库顶点格式 topics_v.csv
"""
...
class TopicsVertexMapper:
    ...
    def __init__(self, config_file="mapping_config.json"):
        ...
        logger.info(f"Topics顶点映射器初始化完成")
    ...
    def map_topic_to_vertex(self, topic_row):
        """将topic行映射为顶点格式
        
        Args:
            topic_row: 原始topic CSV行（字典）
        ...
        # 构建properties JSON对象
        properties = {
            'display_name': topic_row.get('display_name', ''),
            'keywords': topic_row.get('keywords', ''),
            'works_count': topic_row.get('works_count', '0'),
            'cited_by_count': topic_row.get('cited_by_count', '0')
        }
    ...
    def process_topics(self, batch_size=100000):
        """处理topics文件并生成顶点文件"""
        logger.info(f"\n{'='*60}")
        logger.info(f"开始映射 topics.csv -> topics_v.csv")
        logger.info(f"{'='*60}")
        
        input_file = self.input_dir / 'topics.csv'
        output_file = self.output_base_dir / 'topics_v.csv'
            ...
            with open(input_file, 'r', encoding='utf-8') as inf:
                ...
                with open(output_file, 'w', encoding='utf-8', newline='') as outf:
                    ...
                    for i, row in enumerate(reader, 1):
                        try:
                            # 映射为顶点格式
                            vertex = self.map_topic_to_vertex(row)
                            batch.append(vertex)
