...
# -*- coding: utf-8 -*-
"""
Works Authorships Edge 模式映射脚本
将 works_authorships.csv 映射为边数据格式 work_author_e.csv
边类型: work -> author (论文指向作者)
"""
...
class WorksAuthorshipsEdgeMapper:
    """Works Authorships 到边数据格式的映射器"""
    ...
    def __init__(self, config_file="mapping_config.json"):
        ...
        logger.info(f"Works Authorships边映射器初始化完成")
    ...
    def map_to_edge(self, row):
        """将works_authorships行映射为边数据格式
        
        Args:
            row: 原始works_authorships CSV行（字典）
        ...
        if author_position:
            properties['author_position'] = author_position
        
        return {
            'startid': row.get('work_id', ''),       # 起始节点：论文ID
            'endid': row.get('author_id', ''),       # 终止节点：作者ID
            'properties': json.dumps(properties, ensure_ascii=False)
        }
    
    def process_works_authorships(self, batch_size=100000):
        """处理works_authorships文件并生成边数据文件
        
        Args:
            batch_size: 批量写入大小
        """
        logger.info(f"\n{'='*60}")
        logger.info(f"开始映射 works_authorships.csv -> work_author_e.csv")
        logger.info(f"{'='*60}")
        
        input_file = self.input_dir / 'works_authorships.csv'
        output_file = self.output_base_dir / 'work_author_e.csv'
    ...
    def run(self):
        """运行映射流程"""
        logger.info(f"\n{'#'*80}")
        logger.info(f"Works Authorships 边数据映射工具")
        logger.info(f"{'#'*80}\n")
        
        success = self.process_works_authorships()
        
        if success:
            logger.info(f"\n✓ Works Authorships 边数据映射完成！")
        else:
            logger.error(f"\n✗ Works Authorships 边数据映射失败")
...
def main():
    """主函数"""
    mapper = WorksAuthorshipsEdgeMapper()
    mapper.run()
